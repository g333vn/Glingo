...
function JLPTLevelN5Page() {
    ...
    const [jlptN5Exams, setJlptN5Exams] = useState([]);
    ...
    useEffect(() => {
        const loadExams = async () => {
            ...
            const savedExams = await storageManager.getExams('n5');
            // ✅ Load from static file as well
            const defaultExams = jlptExams.n5 || [];
            ...
            setJlptN5Exams(sortExamsByYear(mergedExams));
            console.log(`✅ Loaded ${mergedExams.length} exams (${savedExams?.length || 0} from storage + ${defaultExams.length} from static file) for N5 (sorted by year)`);
        };
    ...
    const paginationData = useMemo(() => {
        ...
        const currentExams = jlptN5Exams.slice(startIndex, endIndex);
        const totalPages = Math.ceil(jlptN5Exams.length / examsPerPage);
        return { startIndex, endIndex, currentExams, totalPages };
    }, [jlptN5Exams, currentPage]);
    ...
    const handleExamClick = useCallback((examId) => {
        const exam = jlptN5Exams.find(e => e.id === examId);
        ...
        navigate(`/jlpt/n5/${examId}`);
    }, [jlptN5Exams, navigate]);
    ...
    const breadcrumbPaths = useMemo(() => [
        ...
        { name: t('jlpt.n5Title') || 'N5', link: '/jlpt/n5' }
    ], [t]);
...
export default JLPTLevelN5Page;
