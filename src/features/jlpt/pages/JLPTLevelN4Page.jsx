...
function JLPTLevelN4Page() {
    ...
    const [jlptN4Exams, setJlptN4Exams] = useState([]);
    ...
    useEffect(() => {
        const loadExams = async () => {
            ...
            const savedExams = await storageManager.getExams('n4');
            // ✅ Load from static file as well
            const defaultExams = jlptExams.n4 || [];
            ...
            setJlptN4Exams(sortExamsByYear(mergedExams));
            console.log(`✅ Loaded ${mergedExams.length} exams (${savedExams?.length || 0} from storage + ${defaultExams.length} from static file) for N4 (sorted by year)`);
        };
    ...
    const paginationData = useMemo(() => {
        ...
        const currentExams = jlptN4Exams.slice(startIndex, endIndex);
        const totalPages = Math.ceil(jlptN4Exams.length / examsPerPage);
        return { startIndex, endIndex, currentExams, totalPages };
    }, [jlptN4Exams, currentPage]);
    ...
    const handleExamClick = useCallback((examId) => {
        const exam = jlptN4Exams.find(e => e.id === examId);
        ...
        navigate(`/jlpt/n4/${examId}`);
    }, [jlptN4Exams, navigate]);
    ...
    const breadcrumbPaths = useMemo(() => [
        ...
        { name: t('jlpt.n4Title') || 'N4', link: '/jlpt/n4' }
    ], [t]);
...
export default JLPTLevelN4Page;
