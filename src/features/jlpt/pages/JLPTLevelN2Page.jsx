...
function JLPTLevelN2Page() {
    ...
    useEffect(() => {
        const loadExams = async () => {
            ...
            setJlptN2Exams(sortExamsByYear(mergedExams));
            console.log(`✅ Loaded ${mergedExams.length} exams (${savedExams?.length || 0} from storage + ${defaultExams.length} from static file) for N2 (sorted by year)`);
        };
    ...
    const paginationData = useMemo(() => {
        ...
        const currentExams = jlptN2Exams.slice(startIndex, endIndex);
        const totalPages = Math.ceil(jlptN2Exams.length / examsPerPage);
        return { startIndex, endIndex, currentExams, totalPages };
    }, [jlptN2Exams, currentPage]);
    ...
    const handleExamClick = useCallback((examId) => {
        const exam = jlptN2Exams.find(e => e.id === examId);
        ...
        navigate(`/jlpt/n2/${examId}`);
    }, [jlptN2Exams, navigate]);
    ...
    const breadcrumbPaths = useMemo(() => [
        ...
        { name: t('jlpt.n2Title') || 'N2', link: '/jlpt/n2' }
    ], [t]);
...
export default JLPTLevelN2Page;
