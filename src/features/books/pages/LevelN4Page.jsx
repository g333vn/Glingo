...
import { n4BooksMetadata } from '../../../data/level/n4/index.js';
import storageManager from '../../../utils/localStorageManager.js';
import { useLanguage } from '../../../contexts/LanguageContext.jsx';

const booksPerPage = 10;

function LevelN4Page() {
    ...
    useEffect(() => {
        const loadBooks = async () => {
            ...
            const savedBooksRaw = await storageManager.getBooks('n4');
            ...
            try {
                const seriesList = await storageManager.getSeries('n4');
                ...
                console.warn('[LevelN4Page] ⚠️ Could not load series for category mapping:', err);
            }

            if (booksWithCategory && booksWithCategory.length > 0) {
                setN4Books(booksWithCategory);
                // Ghi đè lại storage để xoá sạch demo/extra cũ và lưu category đã khôi phục
                await storageManager.saveBooks('n4', booksWithCategory);
                console.log(`✅ Loaded ${booksWithCategory.length} N4 books (demo/extra removed, categories synced)`);
            } else {
                // 2. Không có data trong storage → dùng metadata mặc định (đã được clean)
                const cleanedDefaults = filterDemoAndExtraBooks(n4BooksMetadata);
                setN4Books(cleanedDefaults);
                await storageManager.saveBooks('n4', cleanedDefaults);
                console.log(`📁 Loaded ${cleanedDefaults.length} N4 books from static file (demo/extra removed)`);
            }
        };
        loadBooks();
    }, []);
    ...
    const filteredBooks = selectedCategory
        ? n4Books.filter(book => book.category === selectedCategory)
        : n4Books;
