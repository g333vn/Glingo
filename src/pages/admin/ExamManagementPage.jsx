...
function ExamManagementPage() {
  ...
  const loadExams = async () => {
    // 1 Ưu tiên lấy từ local cache (IndexedDB/localStorage)
    ...
    // 2 Nếu local trống, thử lấy từ Supabase (nguồn chung cho toàn hệ thống)
    try {
    ...
    // 3 Fallback cuối cùng: dữ liệu tĩnh (nếu còn)
  ...
  const loadExamData = async () => {
    ...
    // 1 Ưu tiên lấy exam đầy đủ từ local cache (nếu đã có)
    let data = await storageManager.getExam(selectedLevel, selectedExam.id);

    // 2 Nếu local chưa có dữ liệu chi tiết, thử load từ Supabase
    if (!data) {
      try {
