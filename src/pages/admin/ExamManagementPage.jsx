...
function ExamManagementPage() {
  ...
  const [questionForm, setQuestionForm] = useState({
    ...
    audioPath: '', // ✅ NEW: Audio path in Supabase Storage
    audioName: '', // ✅ NEW: Original audio file name
    audioFile: null
  });
  ...
  const handleSaveSection = async (e) => {
    ...
    if (editingSection) {
      ...
      updatedSections[index] = {
        ...
        // ✅ UPDATED: Keep existing timeLimit when editing (it's managed at level config)
        // Only update if levelConfig changed, otherwise keep original
        timeLimit: editingSection.timeLimit || getDefaultTimeLimit(selectedTestType) || null,
      ...
      updatedSections.push({
        ...
        title: finalTitle, // ✅ Use split title
        instruction: instruction, // ✅ Use split instruction
        // ✅ UPDATED: Time limit is automatically set from levelConfig, not from form
        // Get from levelConfig if available, otherwise null
        timeLimit: getDefaultTimeLimit(selectedTestType) || null,
        questions: []
      });
    }
  ...
  const generateQuestionJSON = () => {
    ...
    const questionData = {
      ...
      ...(selectedTestType === 'listening' && questionForm.audioUrl && {
        ...
        audioName: questionForm.audioName || ''
      })
    };
  ...
  const handleEditQuestion = (section, question) => {
    ...
    setQuestionForm({
      ...
      audioPath: question.audioPath || '', // ✅ NEW
      audioName: question.audioName || '', // ✅ NEW
      audioFile: null
    });
    setExportedJSON('');
    setShowPreview(false);
  ...
  const saveQuestionData = async () => {
    ...
    // ✅ NEW: Upload audio file to Supabase Storage BEFORE processing sections
    let uploadedAudioUrl = null;
    if (selectedTestType === 'listening' && questionForm.audioFile && questionForm.audioUrl?.startsWith('blob:')) {
      setIsUploadingAudio(true);
      try {
        // ✅ Upload to Supabase Storage (same as lesson audio upload)
        const { uploadAudio, generateFilePath } = await import('../../services/fileUploadService.js');
        
        // 📁 Đường dẫn có ngữ nghĩa: level / exam / listening / section / question
        const safeLevel = selectedLevel || 'unknown-level';
        const safeExamId = selectedExam?.id || 'unknown-exam';
        const safeSectionId = selectedSection?.id || 'unknown-section';
        const safeQuestionId = questionForm.id || 'unknown-question';
        const prefix = `level-${safeLevel}/exam-${safeExamId}/listening/${safeSectionId}/${safeQuestionId}`;
        const path = generateFilePath(prefix, questionForm.audioFile.name);
        
        const result = await uploadAudio(questionForm.audioFile, path);
        
        if (result.success) {
          uploadedAudioUrl = result.url;
          // ✅ NEW: Store audio metadata (like Quiz Editor)
          questionForm.audioPath = path;
          questionForm.audioName = questionForm.audioFile.name;
    ...
    const updatedSections = sections.map((section) => {
      if (section.id === selectedSection.id) {
        ...
        // ✅ UPDATED: Determine audio storage format (with metadata like Quiz Editor)
        // Priority: 1. Uploaded Supabase URL, 2. Existing Supabase URL, 3. Existing data URL (backward compatibility), 4. Empty
        let audioStorage = {};
        if (uploadedAudioUrl) {
        ...
        } else if (questionForm.audioUrl && !questionForm.audioUrl.startsWith('blob:') && !questionForm.audioUrl.startsWith('data:')) {
          // Existing Supabase URL (regular URL) - preserve metadata if available
          audioStorage = { 
            audioUrl: questionForm.audioUrl,
            audioPath: questionForm.audioPath || '',
            audioName: questionForm.audioName || ''
          ...
          // Existing audioData loaded as data URL (backward compatibility - keep for old data)
          audioStorage = { audioData: questionForm.audioUrl, audioUrl: '' };
        } else {
          // No audio or invalid blob URL
          audioStorage = { audioUrl: '', audioPath: '', audioName: '' };
        }
  ...
  const templateJSON = useMemo(() => {
    ...
    // ✅ NEW: If section is selected, add a note about section instruction
    // But don't include section instruction in question field
    if (selectedSection) {
      // Add a comment in explanation to remind about section instruction
      if (selectedSection.instruction) {
        questionTemplate.explanation = `[Lưu ý: Instruction của section "${selectedSection.title}" là: "${selectedSection.instruction}" - Được đặt trong Section Form, không phải trong mỗi câu hỏi]\n\n${questionTemplate.explanation}`;
      } else {
        // Section chưa có instruction
        questionTemplate.explanation = `[Lưu ý: Section "${selectedSection.title}" chưa có instruction. Vui lòng thêm instruction trong Section Form]\n\n${questionTemplate.explanation}`;
      }
    } else {
      // Chưa chọn section
      questionTemplate.explanation = `[Lưu ý: Chưa chọn section. Instruction của section sẽ được đặt trong Section Form, không phải trong mỗi câu hỏi]\n\n${questionTemplate.explanation}`;
    }
    ...
    reader.onload = async () => {
      try {
        ...
        if (questionsPayload.length === 0) {
          ...
          // Check listening questions have audio
          if (selectedTestType === 'listening') {
            const missingAudio = normalizedQuestions.filter(q => !q.audioUrl);
            if (missingAudio.length > 0) {
              const confirmContinue = confirm(
                `⚠️ Có ${missingAudio.length} câu hỏi nghe hiểu thiếu audio.\n\n` +
                `Bạn có muốn tiếp tục import không?\n\n` +
                `- OK: Import tất cả (câu thiếu audio sẽ được bỏ qua)\n` +
                `- Cancel: Hủy import`
              );
              if (!confirmContinue) {
                resetInput();
                        ...
                        <div className="relative">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (file) {
                                ...
                                if (!validTypes.includes(file.type)) {
                                  alert(`⚠️ ${t('examManagement.questions.questionForm.selectAudioFile') || 'Chỉ hỗ trợ file audio: MP3, WAV, OGG, M4A'}`);
                                  e.target.value = '';
                                  return;
                                }
                                
                                if (file.size > 10 * 1024 * 1024) {
                                  alert(`⚠️ ${t('examManagement.questions.questionForm.fileTooLarge') || 'File quá lớn! Giới hạn: 10MB'}`);
                                  e.target.value = '';
                                  return;
                                }
                                ...
                                // ✅ Create new blob URL for preview (will upload to Supabase when saving)
                                const audioUrl = URL.createObjectURL(file);
                                console.log('✅ Created blob URL for preview (will upload to Supabase when saving):', audioUrl);
                                setQuestionForm(prev => ({ 
                                  ...prev, 
                                  audioUrl, 
                                  audioFile: file,
                                  audioName: file.name, // ✅ NEW: Store file name
                          ...
                          <div className={`flex items-center gap-2 px-3 py-2.5 border-2 border-purple-300 rounded-lg bg-white ${isUploadingAudio ? 'opacity-50' : ''}`}>
                            ...
                            <span className="text-sm text-gray-600 flex-1">
                              {isUploadingAudio 
                                ? t('examManagement.questions.questionForm.uploading') || 'Đang upload...'
                                : questionForm.audioFile 
                                  ? questionForm.audioFile.name 
                                  : t('examManagement.questions.questionForm.noFileChosen')}
                            </span>
                          </div>
                        </div>
                        <p className="text-xs text-gray-600 mt-2">
                          📎 {t('examManagement.questions.questionForm.selectAudio')} {t('examManagement.questions.questionForm.audioWillUploadOnSave') || '(File sẽ được upload lên Supabase Storage khi lưu câu hỏi)'}
                        </p>
                        {questionForm.audioFile && (
                          <div className="mt-3 p-3 bg-green-50 rounded-lg border-2 border-green-300">
                            ...
                            {questionForm.audioUrl?.startsWith('blob:') && (
                              <p className="text-xs text-orange-600 mt-1 font-semibold">
                                ⚠️ {t('examManagement.questions.questionForm.audioPreviewMode') || 'Chế độ xem trước - File sẽ được upload khi lưu'}
                              </p>
                            )}
                          </div>
                        )}
                        {questionForm.audioUrl && !questionForm.audioUrl.startsWith('blob:') && !questionForm.audioUrl.startsWith('data:') && (
                          <div className="mt-3 p-3 bg-blue-50 rounded-lg border-2 border-blue-300">
                            <p className="text-sm text-blue-800 font-bold flex items-center gap-2">
                              <span>☁️</span>
                              <span>{t('examManagement.questions.questionForm.audioFromSupabase') || 'Audio đã được lưu trên Supabase Storage'}</span>
                            </p>
