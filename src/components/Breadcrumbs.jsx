...
function Breadcrumbs({ paths }) {
  ...
  return (
    <nav className="text-gray-600 mb-4 md:mb-6 text-xs sm:text-sm" aria-label="breadcrumb">
      <ol className="list-none p-0 inline-flex flex-wrap items-center gap-1 sm:gap-2">
            ...
            {index < paths.length - 1 ? (
              ...
              path.onClick ? (
                <button
                  ...
                  className="text-black hover:text-[#FF5722] bg-transparent border-none cursor-pointer font-black uppercase tracking-wide transition-colors hover:bg-yellow-400 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md border-[2px] border-transparent hover:border-black hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-xs sm:text-sm"
                >
                  {path.name}
                </button>
              ) : path.handleNavigate ? (
              ...
              <span className="text-black font-black uppercase tracking-wide text-xs sm:text-sm">{path.name}</span>
            )}
          </li>
        ))}
      </ol>
