...
const CONFIG = {
  ...
  // Layer 2: Folder riêng trên Windows (có thể thay đổi qua env)
  externalBackupDir: process.env.BACKUP_DIR || 'E:\\Projects\\windows_elearning_data',
  
  // Layer 3: Cloud Storage folder - Drive (có thể thay đổi qua env)
...
function copyToBackup(sourceFile, targetDir, backupType) {
  ...
  // Kiểm tra file đã tồn tại chưa (tránh trùng)
  let targetFile = path.join(finalDir, filename);
  let counter = 1;
  while (fs.existsSync(targetFile)) {
    const ext = path.extname(filename);
    const name = path.basename(filename, ext);
    targetFile = path.join(finalDir, `${name}_copy${counter}${ext}`);
    counter++;
  }
...
function processBackupFile(sourceFile) {
  ...
  // Layer 3: Copy vào Cloud Storage folder (nếu có)
  try {
    if (fs.existsSync(CONFIG.cloudBackupDir)) {
      const target3 = copyToBackup(sourceFile, CONFIG.cloudBackupDir, backupType);
      log(`   ✓ Layer 3: ${target3}`, 'green');
      log(`      → File sẽ tự động sync lên cloud`, 'blue');
    } else {
      log(`   ℹ Layer 3: Cloud folder not found`, 'yellow');
      log(`      → Create folder: ${CONFIG.cloudBackupDir}`, 'blue');
      log(`      → Or set CLOUD_DIR environment variable`, 'blue');
    }
  } catch (error) {
    log(`   ✗ Layer 3 failed: ${error.message}`, 'red');
    log(`   ℹ Tip: Create folder manually or check CLOUD_DIR`, 'yellow');
  }
