...
function main() {
  ...
  // Cleanup Layer 3 (Cloud - chỉ cleanup local, không xóa cloud)
  const result3 = cleanupBackups(CONFIG.cloudBackupDir, dryRun);
  ...
  log(`\nLayer 2 (E:\\Projects\\windows_elearning_data):`, 'cyan');
  log(`   Total: ${result2.total} files`, 'blue');
  log(`   Deleted: ${result2.deleted} files`, 'yellow');
  log(`   Freed: ${result2.freed} MB`, 'green');
  
  log(`\nLayer 3 (G:\\Drive của tôi\\drive_elearning_data):`, 'cyan');
  log(`   Total: ${result3.total} files`, 'blue');
  log(`   Deleted: ${result3.deleted} files`, 'yellow');
  log(`   Freed: ${result3.freed} MB`, 'green');
  log(`   ⚠️  Note: Cloud files will sync deletion`, 'yellow');
  
  const totalFreed = (parseFloat(result1.freed) + parseFloat(result2.freed) + parseFloat(result3.freed)).toFixed(2);
